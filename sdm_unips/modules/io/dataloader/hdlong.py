...
For one __getitem__ call we:
    ...
    2. Sample K (default 3..6 per the paper) composite renders. Each render
       is a Dirichlet (alpha,beta,gamma) mix of one randomly chosen point,
       directional, and environment light image, each pre-normalized by its
       per-scene `light_means.config` value and masked.
...
class HdlongLoader:
    ...
    After `load()` the loader exposes the same surface as
